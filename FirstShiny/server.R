...
function(input, output, session) {
  ...
  observeEvent(input$submit, {
    updateSliderInput(session, "bins", max = input$nbins)
  })
  ...
  output$distPlot <- renderPlot({
    ...
    output$mytext <- renderText({
      info <- data()
      paste0("The value of the slider is", input$bins, "The alpha parameter is", 
             info$alpha, "The first data value is", info$ran[1])
    })
